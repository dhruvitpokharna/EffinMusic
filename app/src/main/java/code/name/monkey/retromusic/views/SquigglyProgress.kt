...
// Simple replacement for lerp
private fun lerp(a: Float, b: Float, t: Float): Float {
    return a + (b - a) * t
}

// Inverse linear interpolation
private fun lerpInv(a: Float, b: Float, v: Float): Float {
    return (v - a) / (b - a)
}

// Inverse lerp but clamped to [0,1]
private fun lerpInvSat(a: Float, b: Float, v: Float): Float {
    return MathUtils.clamp(lerpInv(a, b, v), 0f, 1f)
}

// Replacement for Interpolators (use Android built-ins)
// Replacement for SystemUI Interpolators
private object Interpolators {
    // Normal decelerate (default system one)
    val STANDARD_DECELERATE = android.view.animation.DecelerateInterpolator()

    // A stronger decelerate curve (factor > 1 makes it ease-out more)
    val EMPHASIZED_DECELERATE = android.view.animation.DecelerateInterpolator(2f)

    // Linear (fallback if you ever need it)
    val LINEAR = LinearInterpolator()
}


// Stub out traceSection (from SystemUI, not needed in apps)
